	...
	, _useGradient(false)
{
	...
	{
		Log::wprint(ELL_ERROR, L"GUIÆ¤·ô¼ÓÔØÊ§°Ü: Æ¤·ôÂ·¾¶ÃûÎª¿Õ");
		return;
	}
	...
	{
		Log::wprint(ELL_ERROR, L"GUIÆ¤·ô¼ÓÔØÊ§°Ü: skinFilePath=%s", skinFilePath.c_str());
		return;
	}
	...
	if (NULL == pRoot)
	{
		Log::wprint(ELL_ERROR, L"GUIÆ¤·ô¼ÓÔØÊ§°Ü: ¿ÕÎÄ¼þ");
		return;
	}
