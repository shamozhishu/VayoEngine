...
#if 0
	// 测试轨迹线
	ManualObject* pTestObj = pSceneMgr->findObject<ManualObject>(modelName + L"_测试轨迹线");
	if (NULL == pTestObj)
	{
		pTestObj = pSceneMgr->createObject<ManualObject>(modelName + L"_测试轨迹线");
		parent->attachObject(pTestObj);
		pTestObj->setMaterial(L"examples/点云");
		pTestObj->begin(EPT_LINE_STRIP, L"examples/点云");
	...
	: m_sceneMgr(pSceneMgr)
{
	...
	pObj->begin(EPT_LINE_STRIP, L"examples/点云");
	pObj->colour(255, 0, 0);
	...
	: m_sceneMgr(pSceneMgr)
{
	...
	pEnt->setMesh(Root::getSingleton().getMeshManager()->createCubeMesh(Vector3df(10, 10, 10), L"examples/箱子"));
