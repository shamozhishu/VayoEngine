	...
	float frontLength, SceneNode* parent,const wstring& modelName /*= L""*/)
{
	...
	const float angleStep = (M3D_PI * 2.0f) / m3dDegToRad(cnt);
...
BoxModel::BoxModel(SceneManager* pSceneMgr)
	: m_sceneMgr(pSceneMgr)
{
	Entity* pEnt = m_sceneMgr->findObject<Entity>(Root::getSingleton().getLanguage()->getLanguageByID(21));
	...
	pEnt = m_sceneMgr->findObject<Entity>(Root::getSingleton().getLanguage()->getLanguageByID(22));
	...
	pEnt = m_sceneMgr->findObject<Entity>(Root::getSingleton().getLanguage()->getLanguageByID(23));
	pEnt->setMesh(Root::getSingleton().getMeshManager()->createCubeMesh(Vector3df(10, 10, 10), L"examples/Ïä×Ó"));
	pEnt->getSubEntity(0)->setRenderPriority(0);
	pEnt = m_sceneMgr->findObject<Entity>(Root::getSingleton().getLanguage()->getLanguageByID(24));
	pEnt->setMesh(Root::getSingleton().getMeshManager()->createCubeMesh(Vector3df(11, 11, 11), L"examples/Ãè±ß"));
	pEnt->getSubEntity(0)->setRenderPriority(1);
	ManualObject* pObj = m_sceneMgr->findObject<ManualObject>(Root::getSingleton().getLanguage()->getLanguageByID(25));
	pObj->setMaterial(L"examples/Ïä×Ó");
	pObj->generateSphere(6, 30, 30, L"examples/Ïä×Ó");
	pObj->setRenderPriority(0);
	pObj = m_sceneMgr->findObject<ManualObject>(Root::getSingleton().getLanguage()->getLanguageByID(26));
	pObj->setMaterial(L"examples/Ãè±ß");
	pObj->generateSphere(6.5f, 30, 30, L"examples/Ãè±ß");
	pObj->setRenderPriority(1);
	pObj = m_sceneMgr->findObject<ManualObject>(Root::getSingleton().getLanguage()->getLanguageByID(27));
	pObj->setMaterial(L"examples/Ïä×Ó");
	pObj->generateSphere(6, 30, 30, L"examples/Ïä×Ó");
	pObj->setRenderPriority(0);
	pObj = m_sceneMgr->findObject<ManualObject>(Root::getSingleton().getLanguage()->getLanguageByID(28));
	pObj->setMaterial(L"examples/Ãè±ß");
	pObj->generateSphere(6.5f, 30, 30, L"examples/Ãè±ß");
	pObj->setRenderPriority(1);
}
...
BoxModel::~BoxModel()
{
	m_sceneMgr->destroyObject(Root::getSingleton().getLanguage()->getLanguageByID(21));
	m_sceneMgr->destroyObject(Root::getSingleton().getLanguage()->getLanguageByID(22));
	m_sceneMgr->destroyObject(Root::getSingleton().getLanguage()->getLanguageByID(23));
	m_sceneMgr->destroyObject(Root::getSingleton().getLanguage()->getLanguageByID(24));
	m_sceneMgr->destroyObject(Root::getSingleton().getLanguage()->getLanguageByID(25));
	m_sceneMgr->destroyObject(Root::getSingleton().getLanguage()->getLanguageByID(26));
	m_sceneMgr->destroyObject(Root::getSingleton().getLanguage()->getLanguageByID(27));
	m_sceneMgr->destroyObject(Root::getSingleton().getLanguage()->getLanguageByID(28));
}
