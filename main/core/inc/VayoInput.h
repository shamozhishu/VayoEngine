...
* 鼠标和键盘输入处理
\*************************************************************************/
#ifndef __VAYO_INPUT_H__
#define __VAYO_INPUT_H__
...
class _VayoExport Touch
{
public:
	Touch() { reset(); }
	const Position2di& getCurPos() const { return _curPoint; }
	const Position2di& getPrePos() const { return _prePoint; }

	Position2di getDelta() const
	{
		return Position2di(_curPoint._x - _prePoint._x, _curPoint._y - _prePoint._y);
	}
	void setTouchPoint(int x, int y)
	{
		...
		if (!_startPointMark)
		{
			_startPoint = _prePoint = _curPoint;
			_startPointMark = true;
		}
	}
	void reset()
	{
		_startPointMark = false;
		_startPoint = _curPoint = _prePoint = Position2di(0, 0);
	}
...
public:
	...
	virtual bool touchWheel(const Touch& touch, float wheel)  {(void)touch;(void)wheel;return false;}
...
class _VayoExport KeypadDispatcher : public Singleton<KeypadDispatcher>
{
...
class _VayoExport TouchDispatcher : public Singleton<TouchDispatcher>
{
