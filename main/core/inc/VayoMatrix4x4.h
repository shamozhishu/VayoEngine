...
namespace Vayo {
	...
	static const Matrix4x4 IdentityMat4;
	...
	inline Vector3df Matrix4x4::getScale() const
	{
		...
		if (isZero(_m[1]) && isZero(_m[2]) && isZero(_m[4]) &&
			isZero(_m[6]) && isZero(_m[8]) && isZero(_m[9]))
			return Vector3df(_m[0], _m[5], _m[10]);

		// we have to do the full calculation.
		return Vector3df(sqrtf(_m[0] * _m[0] + _m[1] * _m[1] + _m[2] * _m[2]),
			sqrtf(_m[4] * _m[4] + _m[5] * _m[5] + _m[6] * _m[6]),
			sqrtf(_m[8] * _m[8] + _m[9] * _m[9] + _m[10] * _m[10]));
	}
	...
	{
		setRotationRadians(VayoDegToRad(angle), axis);
	}
	...
	inline Vector3df Matrix4x4::getRotationDegrees() const
	{
		...
		if (!isZero(c))
		{
			...
			x = VayoRadToDeg(atan2(roty, rotx));
			rotx = mat[0] * invC * invScale._x;
			roty = mat[1] * invC * invScale._x;
			z = VayoRadToDeg(atan2(roty, rotx));
		}
		else
		{
			x = 0.0;
	...
	{
		Vector3df tmp = vect;
		vect._x = tmp._x*_m[0] + tmp._y*_m[4] + tmp._z*_m[8];
		vect._y = tmp._x*_m[1] + tmp._y*_m[5] + tmp._z*_m[9];
		vect._z = tmp._x*_m[2] + tmp._y*_m[6] + tmp._z*_m[10];
	}
