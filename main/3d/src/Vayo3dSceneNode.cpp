...
#include "Vayo3dWireBoundingBox.h"

NS_VAYO3D_BEGIN

Reflex<SceneNode, const wstring&, Node*, SceneManager*> SceneNode::_dynReflex;
SceneNode::SceneNode(const wstring& name, Node* parent, SceneManager* oriSceneMgr)
	: Node(name, parent, oriSceneMgr)
...
void SceneNode::visit(float dt)
{
	if (isCanVisit())
	{
		...
		animating(dt);
		updateWorldAABB();
		if (!_oriSceneMgr->isCulled(this))
		{
			if (_showBoundingBox)
				_oriSceneMgr->registerForRendering(_wireBoundingBox, ERQ_WIRE_BOUNDING_BOX);
...
{
	if (!obj || obj->getOriSceneMgr() != getOriSceneMgr())
		return;
...
SceneNode* SceneNode::createChildSceneNode(const wstring& name /*= L""*/)
{
	SceneNode* pRet = NULL;
	if (_oriSceneMgr)
		pRet = _oriSceneMgr->createSceneNode<SceneNode>(this, name);
	return pRet;
}
