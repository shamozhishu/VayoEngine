	...
	unsigned int polyCountX, unsigned int polyCountY, const wstring& materialName /*= L""*/)
{
	...
	const double angleX = 2 * M3D_PI / polyCountX;
	const double angleY = M3D_PI / polyCountY;
		...
		for (unsigned int xz = 0; xz < polyCountX; ++xz)
		{
			...
			if (y == 0)
			{
				if (normal._y != -1.0f && normal._y != 1.0f)
					tu = static_cast<float>(acos(clamp_(normal._x / sinay, -1.0, 1.0)) * 0.5 *M3D_RECIPROCAL_PI);
				if (normal._z < 0.0f)
					tu = 1 - tu;
			}
			else
			...
			pSubMesh->setVertex(Vertex(pos._x, pos._y, pos._z,
				...
				tu, static_cast<float>(ay*M3D_RECIPROCAL_PI)), i);
			++i;
			axz += angleX;
		}
	...
	float oblique /*= 0.0f*/, const wstring& materialName /*= L""*/)
{
	...
	const float angleStep = (M3D_PI * 2.f) * recTesselation;
	...
	const wstring& materialName /*= L""*/)
{
	...
	const float angleStep = (M3D_PI * 2.f) / tesselation;
	const float angleStepHalf = angleStep*0.5f;
