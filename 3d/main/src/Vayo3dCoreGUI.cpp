...
bool UIImageSet::init(const wstring& filepath)
{
	...
	_imagePath = filePaths.ImgsetPath;
...
bool UIManager::init()
{
	const tagUIConfig& uiConfig = ConfigManager::getSingleton().getUIConfig();
	do
	{
	...
	const vector<tagFontConfig>& fontsSet = ConfigManager::getSingleton().getFontConfig();
	...
	for (; cit != fontsSet.cend(); ++cit)
	{
		...
		const tagFontConfig& curFont = *cit;
		string fontPath = w2a_(curFont.FontFilePath);
		int fontID = s_fontManager->createFont(fontPath.c_str(), 0, curFont.FontSize, curFont.FontBold, curFont.FontItalic, true);
		if (fontID == -1)
		{
